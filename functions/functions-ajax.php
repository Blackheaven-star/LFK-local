...
function l4k_loginToLibrary() {
    ...
    if ($overallMatched) 
    {
		...
		// record event 900 for member login (web_activity table) - fully incremental
		l4k_addWebActivity('900'); 
		...
		if ($_POST['remember']) 
		{
		    ...
		    setcookie('remember_member', base64_encode($cookie_data), time() + 60*60*24*30, '/', '', false, true ); // cookie expires in 30 days
		}
    ...
    { 
    	...
    	if ($alertCode == '1062') {
	    	$dataArr = array(	
				...
				'Language' => get_the_title($_POST['language']),
				'Type' => $_POST['type']);    		
    	}
