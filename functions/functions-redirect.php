...
function l4k_checkAuthAutoLogin() {
	...
	if (isset($_GET['auth'])) 
    {
		...
		if ((!empty($library)) && (get_field('auto_login_status', $library[0]->ID))) 
		{
			...
			$ownerID = l4k_addUserLoginActivity(
				'900', 
				strtoupper(get_the_title($library->ID) . ' Auto Login'),
	    	...
	    	$_SESSION['library_barcode'] 		= strtoupper(get_the_title($library->ID) . ' Auto Login');
	...
	if ($_GET['custID']) {
	    if (have_rows('domain_whitelist', 'option')) {
	        while (have_rows('domain_whitelist', 'option')) {
	            ...
	            the_row();
	        	if (get_sub_field('identifier') == $_GET['custID']) {
...
add_filter('query_vars', function($vars) {
    return array_merge($vars, ['library', 'trial', 'competition', 'dashboard']);
});
