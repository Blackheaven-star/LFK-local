    ...
    if($isReadingPack) {
		...
		$tier1 = get_posts([
		    ...
		    'meta_query'     => [
		        ...
		        // add this to include posts without the views meta
		        'views_clause' => [
		            'relation' => 'OR',
		    ...
		    'orderby' => ['views_clause' => 'DESC', 'rand' => 'DESC'],
		]);
        ...
        if ($remaining > 0) {
			...
			$tier2 = get_posts([
			    ...
			    'meta_query'     => [
			        ...
			        ['key' => 'featured', 'value' => 1],
			        ['key' => 'language', 'value' => $langID],
			        ['key' => 'levels_tier','value' => 2, 'compare' => '=', 'type' => 'NUMERIC'],
			        // add this to include posts without the views meta
			        'views_clause' => [
			            'relation' => 'OR',
			            ['key' => 'additional_details_views_last_3_months', 'compare' => 'EXISTS'],
...
function l4k_getBookDetails($bookID) {
    ...
    $permalink = get_field('book_permalink', $bookID);
	$audioFiles = [];
	$audioFilesBilingual = [];
	$audioPlay = [];
	$coverImages = [];
	...
	if (have_rows('cover', $bookID)) {
    	while (have_rows('cover', $bookID)) {
        	the_row();
	        $coverImages[] = [
    ...
    $bookButtons[] = array(	
        '_button_name' 		=> $langDetails['variant_label'][$type], 
        'book_type' 		=> $type, 
		'_button_content'   => get_field('details_description', $bookID),
		'_button_title'     => get_field('native_title', $bookID),
		'_button_image'		=> get_field('book_image_url', $bookID),
		'_button_link'		=> get_permalink($bookID),
        '_button_vimeo_video_link' 	=> get_field('book_permalink', $bookID),
		'is_double_page'    => (bool) get_field('is_double_page', $bookID));	

    $video_sources[] = array(
        'views'             => get_field('additional_details_views', $bookID),
        'type'              => $type,
		'vimeo'             => $type === 'vimeo' ? $permalink : null,
	...
	$video_sources_1[] = array(       
		...
		'cover_image_2'     => $coverImages[1] ?? null,
	...
	$bookDetails = [
		...
		'title'		        => get_the_title(get_field('native_story', $bookID)),
		'title_language'	=> get_field('native_title', $bookID),
		'content'           => get_field('details_description', $bookID),
        ...
        'button_bilingual_play' => "",
        'button_bilingual_pause' => "",
        'button_play'       => "",
        'button_pause'      => "",		
...
function l4k_getMobileAppNotification($library_name, $device_id, $os_type, $device_type)
{
	...
	$notification = get_option('latest_notif_details');
    return array(
        'library_name' => $result->library_name,
        'device_id' => $result->device_id,
